...
class AdminController extends Controller
{
    ...
    function dashboard(){
        ...
        return view('dashboard.dashboard')->with('demtb',$demtb)->with('dembc',$dembc)->with('demnv',$demnv)->with('demspondinh',$demspondinh)->with('demspdanghong',$demspdanghong)->with('demspdangsua',$demspdangsua);

        
    }
    function fix(){
        $demspdh = DB::select('SELECT loaithietbi.loai,SUM(soluong) as dem,thietbi.ID_loai FROM thietbi INNER JOIN loaithietbi ON loaithietbi.ID_loai = thietbi.ID_loai WHERE tinhtrang="0" GROUP BY loaithietbi.ID_loai');
        return view('/dsspdanghong.dsdh')->with('demspdh',$demspdh);
    }
    function maintance(){
        $demspds = DB::select('SELECT loaithietbi.loai,SUM(soluong) as dem,thietbi.ID_loai FROM thietbi INNER JOIN loaithietbi ON loaithietbi.ID_loai = thietbi.ID_loai WHERE tinhtrang="2" GROUP BY loaithietbi.ID_loai');
